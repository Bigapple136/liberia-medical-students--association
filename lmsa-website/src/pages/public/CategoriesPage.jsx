...
const categories = [
  {
    name: 'Full Member',
    price: '$25/year',
    features: ['All medical students currently enrolled', 'Full voting rights', 'Eligible to run for office', 'Access to all resources', 'Committee participation', 'Event discounts'],
    recommended: true,
  },
  {
    name: 'Associate Member',
    price: '$15/year',
    features: ['Pre-medical and health sciences students', 'Access to events and resources', 'Committee participation', 'No voting rights', 'Cannot hold elected office', 'Pathway to full membership'],
  },
  {
    name: 'Honorary Member',
    price: 'By invitation',
    features: ['Faculty and alumni supporters', 'Recognized contributors', 'Advisory role only', 'No voting rights', 'Cannot hold office', 'Lifetime appointment'],
  },
];
        ...
        <div className="site-container">
          ...
          <div className="grid gap-4 md:grid-cols-3">
            {categories.map((category, index) => (
              <article key={category.name} className={`editorial-link-card ${category.recommended ? 'border-t-4 border-t-lmsa-600' : ''}`}>
                ...
                <Link to="/membership#apply" className={`mt-7 inline-flex items-center justify-center px-4 py-3 text-sm font-semibold ${category.recommended ? 'bg-lmsa-600 text-white hover:bg-lmsa-700' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'}`}>
                  Apply now
                </Link>
              </article>
            ))}
          ...
          <div className="flex items-start gap-4 border-l-2 border-lmsa-600 bg-lmsa-50 p-6">
            ...
            <div>
              ...
              <p className="mt-2 text-base leading-7 text-gray-700">
                Questions about eligibility or member rights? Contact us at{' '}
                <a href="mailto:membership@lmsa.org" className="font-semibold text-lmsa-700 hover:underline">membership@lmsa.org</a>.
              </p>
            </div>
          </div>
        </div>
